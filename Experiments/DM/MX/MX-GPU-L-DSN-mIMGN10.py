...
def conv_block(channels):
    ...
    out.add(
        ...
        gluon.nn.Conv2D(channels, kernel_size=3, padding=1)
    )
    return out

class DenseBlock(gluon.nn.Block):
...
def transition_block(channels):
    ...
    out.add(
        gluon.nn.BatchNorm(),
        gluon.nn.Activation('relu'),
        gluon.nn.Conv2D(channels, kernel_size=1),
...
def dense_net():
    ...
    with net.name_scope():
        # first block
        net.add(
            gluon.nn.Conv2D(init_channels, kernel_size=7,
                      strides=2, padding=3),
            gluon.nn.BatchNorm(),
            gluon.nn.Activation('relu'),
        ...
        net.add(
            gluon.nn.BatchNorm(),
            gluon.nn.Activation('relu'),
            gluon.nn.AvgPool2D(pool_size=1),
            gluon.nn.Flatten(),
