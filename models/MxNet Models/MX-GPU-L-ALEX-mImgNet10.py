...
with net.name_scope():
    net.add(gluon.nn.Conv2D(channels=64, kernel_size=11, strides=4, padding=2, activation='relu'))
    net.add(gluon.nn.MaxPool2D(pool_size=3, strides=2))
    net.add(gluon.nn.Conv2D(channels=192, kernel_size=5, padding=2, activation='relu'))
    net.add(gluon.nn.MaxPool2D(pool_size=3, strides=2))
